...
function getMusicList(callback) {
  ...
  xhr.open(
    "GET",
    "https://itsyaeji1993.github.io/miao/musicPlayer/music.json",
    true
  );
  xhr.onload = function() {
...
$(".control .play").onclick = function() {
  if (audio.paused) {
    audio.play();
    $(".control .play .fa").classList.add("fa-pause");
    $(".control .play .fa").classList.remove("fa-play");
  } else {
    audio.pause();
    $(".control .play .fa").classList.remove("fa-pause");
    $(".control .play .fa").classList.add("fa-play");
  }
};
...
$(".setvolume .volume").onclick = function(e) {
  ...
  $(".setvolume .percentage").innerText = Math.floor(percent * 100) + "%";
};
